...
impl FsDir {
    ...
    pub fn init_ttl_manager(&mut self, ttl_manager: Arc<InodeTtlManager>) -> FsResult<()> {
        ...
        let db_conf = self.conf.meta_rocks_conf();
        let store = RocksInodeStore::new(db_conf, self.conf.format_master)?;
        let state = InodeStore::new(store);
        let (last_inode_id, root_dir) = state.create_tree(self.ttl_manager.clone())?;
