...
impl JournalLoader {
    ...
    async fn apply0(&self, is_leader: bool, entry: &Entry) -> CommonResult<()> {
        ...
        if is_leader {
            ...
            self.journal_writer
                .log_ufs_applied(op_id, applied.term, applied.index)?;
        ...
        if let Some(e) = snapshot {
            ...
            let snap_data =
                RaftUtils::create_file_snapshot(&e.dir, self.node_id, fsm_state.clone())?;

            self.log_store.create_snapshot(snap_data.clone())?;
            self.log_store.compact(fsm_state.compact())?;
        ...
        loop {
            ...
            match apply_msg {
                ApplyMsg::CreateSnapshot(tx) => {
                    if let Err(e) = tx.send(self.create_snapshot0()) {
    ...
    fn create_snapshot0(&self) -> RaftResult<SnapshotData> {
        let fsm_state = self.get_fsm_state();
        let fs_dir = self.fs_dir.read();
    ...
    pub fn symlink(&self, entry: SymlinkEntry) -> CommonResult<()> {
        ...
        match fs_dir.unprotected_symlink(inp, entry.new_inode, entry.force) {
            ...
            Err(FsError::FileAlreadyExists(_)) => {
                warn!("Symlink: file already exists: {:?}", entry);
                Ok(())
            }
            Err(e) => Err(e.into()),
        }
