...
fn test_ttl_cleanup() -> CommonResult<()> {
    ...
    conf.master.ttl_checker_interval_ms = 1000; // 1 second check interval for testing
    conf.master.ttl_bucket_interval_ms = 1000; // 1 second bucket interval
...
async fn test_ttl_file_cleanup(fs: &FsClient, conf: &ClientConf) -> CommonResult<()> {
    ...
    if exists {
        warn!("File still exists after TTL expiration, waiting a bit more...");
        sleep(Duration::from_millis(2000)).await;
        let exists_after_wait = fs.exists(&file_path).await?;
        assert!(
            !exists_after_wait,
            "File should be deleted after TTL expiration"
        );
    } else {
...
async fn test_ttl_directory_cleanup(fs: &FsClient, conf: &ClientConf) -> CommonResult<()> {
    ...
    if exists {
        warn!("Directory still exists after TTL expiration, waiting a bit more...");
        sleep(Duration::from_millis(2000)).await;
        let exists_after_wait = fs.exists(&dir_path).await?;
        assert!(
            !exists_after_wait,
            "Directory should be deleted after TTL expiration"
        );
    } else {
