...
impl ClusterConf {
    ...
    /// validate(section init) → resolve_master_addrs`.
    ...
    pub fn from<T: AsRef<str>>(path: T) -> CommonResult<Self> {
        ...
        let mut conf = Self::try_from_doc(doc)?;
        conf.normalize_whitespace();
        conf.run_section_validation()?;
    ...
    /// Single serde deserialization pass — native semantics (defaults, aliases,
    /// arrays of tables, error messages) preserved verbatim.
    fn try_from_doc(doc: toml::Value) -> CommonResult<Self> {
        Ok(try_err!(doc.try_into()))
    }

    /// Section hard-validation + derived-field init, in the historical order.
    /// Each impl delegates to the section's own init(), keeping one source of
    /// truth for the rules; failures abort the load exactly as before.
    fn run_section_validation(&mut self) -> CommonResult<()> {
...
mod tests {
    ...
    // Full-profile real-path check demanded by review: ClusterConf::from()
    // itself (not a re-implementation of its stages) must resolve every role
    // hostname from net_interface and ignore the hostname env allowlist.
    #[test]
    ...
    // End-to-end layer-priority check at the struct level, mirroring from()'s
    // stages but with an injected env lookup so parallel tests never mutate
    // process-global environment variables.
    #[test]
    fn file_layer_loses_to_env_layer() {
        ...
        let master_env = ClusterConf::ENV_MASTER_HOSTNAME;
        let lookup = |var: &str| (var == master_env).then(|| "env-master".to_string());
        crate::pipeline::apply_env_layer_with(&mut doc, LoadProfile::Full, &lookup).unwrap();

        let mut conf: ClusterConf = doc.try_into().unwrap();
        conf.normalize_whitespace();
        conf.run_section_validation().unwrap();

        assert_eq!(conf.master.hostname, "env-master");
        assert_eq!(conf.journal.hostname, "env-master");
        assert_eq!(conf.client.hostname, "file-client");
    }
    ...
    fn transfer_net_interface_keeps_master_address() {
        ...
        std::fs::write(
            ...
            format!(
                ...
                # Empty membership list: client.master_addrs derives from
                # master.hostname instead of the journal peers.
                [journal]
                journal_addrs = []
            "#
            ),
