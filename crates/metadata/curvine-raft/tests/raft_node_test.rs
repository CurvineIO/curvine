...
impl AppStorage for FailingSnapshotAppStorage {
    async fn apply(&self, _: bool, _: curvine_raft::storage::ApplyMsg) -> RaftResult<()> {
        Ok(())
    }
...
impl AppStorage for TestKvAppStorage {
    async fn apply(&self, _: bool, msg: curvine_raft::storage::ApplyMsg) -> RaftResult<()> {
        match msg {
            curvine_raft::storage::ApplyMsg::Entry(entry) => {
                let pair: (String, String) = SerdeUtils::deserialize(&entry.data)?;
                self.map.write().unwrap().insert(pair.0, pair.1);
                self.fsm_state.lock().unwrap().applied = curvine_raft::proto::AppliedIndex {
                    term: entry.term,
                    index: entry.index,
                    op_id: 0,
                    rpc_id: 0,
            ...
            curvine_raft::storage::ApplyMsg::Scan(applied) => {
