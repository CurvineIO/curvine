    ...
    B: AppStorage,
{
    ...
    pub async fn install_snapshot(log_store: &A, app_store: &B, voters: Vec<u64>) -> RaftResult<u64> {
    ...
    async fn on_ready(&mut self, promise: &mut HashMap<i64, Callback>) -> RaftResult<()> {
        ...
        let mut apply_res = self.apply_committed_entries(ready.take_committed_entries(), promise).await;
        ...
        if apply_res.is_ok() {
            apply_res = self.apply_committed_entries(light_rd.take_committed_entries(), promise).await;
        }

        match apply_res {
            Err(e) => {
                if self.is_leader() {
                    // The leader replays the UFS logs. If it fails, it logs an error and doesn't execute `advance_apply`.
                    // It continues execution on the next run, ensuring idempotency by repeatedly executing `advance_apply`.
                    // This is to solve the following problems:
                    //  1. Removing UFS does not affect Curvine access.
                    //  2. After UFS recovers, it re-executes to ensure no data loss.
