...
use curvine_common::proto::DataHeaderProto;  // 🔑 添加header支持
...
pub struct BlockWriterRemote {
    ...
    seq_id: i32,
    req_id: i64,
    pending_header: Option<DataHeaderProto>,  // 🔑 保存pending的header信息
}
...
impl BlockWriterRemote {
    ...
    pub async fn with_offset(
        ...
        let writer = Self {
            ...
            req_id,
            worker_address,
            pending_header: None,  // 🔑 初始化header字段
        };
    ...
    pub async fn write(&mut self, chunk: DataSlice) -> FsResult<()> {
        ...
        self.client
            .write_data(chunk, self.req_id, next_seq_id, header)  // 🔑 传递header
            .await?;
