...
impl FallbackFsReader {
    ...
    async fn validate_ufs_consistency(&self) -> FsResult<()> {
        ...
        // ufs_mtime == 0 means data was never flushed to UFS; cannot fall back safely.
        if cv_ufs_mtime == 0 {
            return err_box!(
                "Cannot fallback to UFS for {}: data has not been flushed to UFS yet",
                self.ufs_path
            );
        }
        ...
        if cv_ufs_mtime != ufs_status.mtime || cv_len != ufs_status.len {
            return err_box!(
                "UFS data inconsistent for {}: cv_ufs_mtime={}, ufs_mtime={}, cv_len={}, ufs_len={}",
                self.ufs_path,
                cv_ufs_mtime,
                ufs_status.mtime,
                cv_len,
...
/// Pure function for unit-testable UFS consistency check logic.
#[cfg(test)]
