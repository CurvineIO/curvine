...
The proposal keeps the current Prometheus `/metrics` model, reuses `curvine-core::common::Metrics`, and focuses on low-cardinality, statically-labeled metrics that are safe for high-frequency FUSE workloads. The phasing is designed so each step is a small, independently reviewable change that does not touch the existing `dispatch_meta()` opcode matrix in a single large diff.
...
Curvine has a process-global Prometheus registry behind `curvine-core::common::Metrics`:
...
- A `TimeSpent` RAII helper (`curvine-core/src/common/time_spent.rs`) provides drop-guard timers for counters and histogram-vecs.
