...
1. The `metrics_handler` does **only** `Metrics::text_output()`. No state-gauge refresh, no client RPC, no map traversal. **Phase boundary:** this is the *end state* (reached in **Phase 1b-2**, when the legacy gauges become event-driven and `set_metrics()` is removed). **Phase 1b-1 temporarily keeps `state.set_metrics()`** (the scrape-time legacy-gauge refresh) so those gauges stay correct until 1b-2; consequently `metrics_scrape_duration_us` in 1b-1 times only `text_output()` and under-counts the full handler cost by the `set_metrics()` scan. After 1b-2 the duration equals the full handler cost.
...
> - **Failure before the bulk reset** (e.g. a corrupt header/length read before `nodes.clear()`): the live map is untouched, so the gauge keeps the old live count (restore did not start mutating state). For handle gauges, a failure before the handle phase begins likewise leaves them at their old value (this restore did not touch the handle maps).
> - **Failure after the bulk reset / mid handle-load**: the gauge reflects the partial live map count actually loaded so far.
>
> In all cases the gauge equals the current live map count, so operators reading it after a recovery see live state.
